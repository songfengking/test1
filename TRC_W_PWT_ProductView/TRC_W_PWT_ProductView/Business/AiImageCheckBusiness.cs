...
namespace TRC_W_PWT_ProductView.Business {
	...
	public class AiImageCheckBusiness {
		...
		/// <param name="maxRecordCount">最大検索件数</param>
		/// <returns>処理結果</returns>
		public static ImgCheckResultSet SelectSetubiJyohoList( Dictionary<string, object> condition, int maxRecordCount ) {
			//変数定義
			var result = new ImgCheckResultSet();

			// ステーション
			var station = DataUtils.GetDictionaryStringVal( condition, AnlGroupModelList.CONDITION.STATION_NM_LBL.bindField ).Split( ':' )[0];

			// ライン
			var lineCd = DataUtils.GetDictionaryStringVal( condition, AnlGroupModelList.CONDITION.LINE_NM_LBL.bindField ).Split( ':' )[0];

			// 設備
			var setubi = DataUtils.GetDictionaryStringVal( condition, AnlGroupModelList.CONDITION.SETUBILBL.bindField );

			// 画像解析グループコード
			var imgAnlGroupCd = DataUtils.GetDictionaryStringVal( condition, AnlGroupModelList.CONDITION.GROUP_CD_LBL.bindField );
		...
		/// <param name="maxRecordCount">最大検索件数</param>
		/// <returns>処理結果</returns>
		public static ImgCheckResultSet SelectGroupAsyList( Dictionary<string, object> condition, int maxRecordCount ) {
			//変数定義
			var result = new ImgCheckResultSet();

			// 画像解析グループコード
			var txtGroupCd = DataUtils.GetDictionaryStringVal( condition, AnlGroupList.CONDITION.GROUP_CD_LBL.bindField ).Split( ':' )[0];
