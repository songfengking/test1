<%@ Page Language="C#" MasterPageFile="~/UI/MasterForm/MasterMain.master" AutoEventWireup="true" CodeBehind="AnlGroupView.aspx.cs" Inherits="TRC_W_PWT_ProductView.UI.Pages.AiImageCheck.AnlGroupView" %>
...
<asp:Content ID="MasterScript" ContentPlaceHolderID="MasterScript" runat="server">
	<script src="<%: PageUtils.ResolveUrl( this.Page, "~/Scripts/Pages/AiImageCheck/AnlGroupView.js") %>"></script>
...
<asp:Content ID="MasterBody" ContentPlaceHolderID="MasterBody" runat="server">
												...
												<td>
													<table class="table-condition-sub">
														<tr class="font-default">
															...
															<td>
																<KTCC:KTTextBox ID="txtImgGroupCd" runat="server" InputMode="IntNum" MaxLength="10" CssClass="font-default txt-default ime-inactive txt-width-long" AutoUpper="False" RegExpression="^[0-9!-/:-@¥[-`{-~]*$" />
															</td>
														</tr>
														<tr class="font-default">
	...
	<div style="display: none">
		...
		<KTCC:KTButton ID="btnModalDispTmp" runat="server" />
		<KTCC:KTButton ID="btnMessage" runat="server" OnClick="btnMessage_Click" />
		<asp:TextBox ID="txtComment" runat="server" />
		<asp:TextBox ID="txtCommentCopy" runat="server" />
	</div>
	<%-- iframeの呼び出し --%>
...
<asp:Content ID="MasterBodyBottom" ContentPlaceHolderID="MasterBodyBottom" runat="server">
	<div>
		<div id="div1" runat="server" class="div-bottom-button-area">
			<KTCC:KTButton ID="btnModalCopy" runat="server" Text="コピーして追加" CssClass="btn-middle" Style="width: 130px; margin-right: 20px" />
			<KTCC:KTButton ID="btnModalNew" runat="server" Text="新規追加" CssClass="btn-middle" Style="margin-right: 20px" />
			<KTCC:KTButton ID="btnModalDisp" runat="server" Text="画像解析構成" CssClass="btn-middle" Style="width: 130px" OnClick="btnModalDisp_Click" />
		</div>
	</div>
</asp:Content>
