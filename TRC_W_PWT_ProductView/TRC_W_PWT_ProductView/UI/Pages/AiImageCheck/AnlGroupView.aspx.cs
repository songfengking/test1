...
namespace TRC_W_PWT_ProductView.UI.Pages.AiImageCheck {
	public partial class AnlGroupView : BaseForm {
		...
		/// 画像解析グループ検索画面(表示用)
		/// </summary>
		const string FLAG_ANL_GROUP_VIEW_DISP = "画像解析グループ検索画面";
		/// <summary>
		/// 画像解析グループ更新画面(表示用)
		/// </summary>
		const string FLAG_ANL_GROUP_UPDATE_DISP = "画像解析グループ更新";
		/// <summary>
		/// 画像解析グループ新規登録画面(表示用)
		/// </summary>
		const string FLAG_ANL_GROUP_NEW_DISP = "画像解析グループ新規登録";
		/// <summary>
		/// 画像解析グループコピーして追加画面(表示用)
		/// </summary>
		const string FLAG_ANL_GROUP_COPY_DISP = "画像解析グループコピーして追加";
		/// <summary>
		/// オブジェクトダブルクリック
		/// </summary>
		...
		/// クエリ条件リスト
		/// </summary>
		private static Dictionary<string, object> dicCondition = null;
		/// <summary>
		/// 表示用リスト
		/// </summary>
		private static Dictionary<string, string> dicDataList = null;
		/// <summary>
		/// 選択用リスト
		/// </summary>
		public static Dictionary<string, string> dicSelectedRow = null;
		public static Dictionary<string, string> dicDoubleClickRow = null;
		/// <summary>
		/// △(ASC)
		/// </summary>
		...
		/// グリッドビューグループ
		/// </summary>
		const string GRID_ANL_GROUP_VIEW_CD = "AnlGroupView";
		...
		protected override void DoPageLoad( ) {
			...
			SetGridViewTemplateField( grvDetailBodyRB, ControlUtils.GetFrozenColumns( frozenGrid, GridviewDefault, false ) );
		...
		private void InitializeValues() {
			...
			try {
				...
				//ボタン非活性化
				btnModalCopy.Enabled = false;
				btnModalNew.Enabled = false;
		...
		private void DoImgAnl() {
			...
			if ( true == StringUtils.IsEmpty( txtComment.Text ) ) {
				...
				txtComment.Text = "";
				txtCommentCopy.Text = "";
				ScriptManager.RegisterClientScriptBlock( this, this.GetType(), "ControlCommon_DoubleClickGridRow", "$(function(){ ControlCommon.DoubleClickGridRow( 0, 1, 'AnlGroupList.aspx', '" + base.Token + "' ) });", true );
			} 
		}
		#endregion
		...
		private void DoSearch( ) {
			...
			if ( true == StringUtils.IsEmpty( ddlLineCd.SelectedValue ) ) {
				...
				dicCondition["ddlSetubi"] = ddlSetubi.Items[1].Value;
			};
			...
			if ( true == ObjectUtils.IsNotNull( tblResult ) ) {
				if ( 0 < tblResult.Rows.Count ) {
					...
					ShowGridViewHeader( grvDetailHeaderRT, ControlUtils.GetFrozenColumns( frozenGrid, GridviewDefault, false ), ConditionInfo, true );
					...
					BindGridView_WithTempField( grvDetailBodyRB, ControlUtils.GetFrozenColumns( frozenGrid, GridviewDefault, false ), tblResult );
				...
				/// 権限による制御
				/// </summary>
				AppPermission.PERMISSION_INFO permAuthInfo = AppPermission.GetTransactionPermission( TRC_W_PWT_ProductView.Defines.PageInfo.AnlGroupInput, LoginInfo.UserInfo );

				//権限取得
				if ( permAuthInfo.IsEdit == true ) {
					//ボタン活性化
					btnModalNew.Enabled = true;
					btnModalCopy.Enabled = true;
				} else {
					//ボタン非活性化
					btnModalNew.Enabled = false;
					btnModalCopy.Enabled = false;
				...
				liArr[1] = new ListItem( "txtLineNm", ddlLineCd.SelectedValue );
				liArr[2] = new ListItem( "txtStationNm", ddlStation.SelectedValue );
				liArr[3] = new ListItem( "txtLineCd", dicCondition["ddlLineCd"].ToString() );
				liArr[4] = new ListItem( "txtStationCd", dicCondition["ddlStation"].ToString() );
				liArr[5] = new ListItem( "txtSetubi", dicCondition["ddlSetubi"].ToString() );
				btnModalNew.Attributes[ControlUtils.ON_CLICK] = AiImageCheck.ImageInputModal.ImageCreateDispUrl( this, PageInfo.AnlGroupInput, 10, 10, liArr, FLAG_ANL_GROUP_NEW_DISP, "input" );
			}
		...
		private void RowDataBound( params object[] parameters ) {
			...
			if ( e.Row.RowType == DataControlRowType.DataRow ) {
				...
				// データ行のみバインド処理を行う
				int index = 0;
				ControlUtils.GridViewRowBound( sender, e, GRID_ANL_GROUP_VIEW_CD, ControlUtils.GridRowDoubleClickEvent.None );
				...
				dict.Add( "txtLineCd", ( (DataRowView)e.Row.DataItem ).Row[GRID_SEARCHDETAIL.LINE_CD.bindField].ToString() );
				dict.Add( "txtStationCd", ( (DataRowView)e.Row.DataItem ).Row[GRID_SEARCHDETAIL.STATION_CD.bindField].ToString() );
				e.Row.Attributes[ControlUtils.ON_CLICK] = e.Row.Attributes[ControlUtils.ON_CLICK] + "AnlGroupView.TxtWrite1('" + ParseToString( dict ) + "');";
				int idx = 1;
				...
				e.Row.Attributes[ControlUtils.ON_CLICK] = e.Row.Attributes[ControlUtils.ON_CLICK] + "return AnlGroupView.TxtWrite('" + ParseToString( dict ) + "');";
				...
				if ( permAuthInfo.IsEdit == true ) {
					//ダブルクリック用
					//keyの作成
					ListItem[] liArr = new ListItem[8];
					liArr[0] = new ListItem( "exeKbn", "2" );
					liArr[1] = new ListItem( "txtGroupCd", dict["txtGroupCd"].ToString() );
					liArr[2] = new ListItem( "txtGroupNm", dict["txtGroupNm"].ToString() );
					liArr[3] = new ListItem( "txtSetubi", dict["txtSetubi"].ToString() );
					liArr[4] = new ListItem( "txtLineNm", dict["txtLineNm"].ToString() );
					liArr[5] = new ListItem( "txtStationNm", dict["txtStationNm"].ToString() );
					liArr[6] = new ListItem( "txtLineCd", dict["txtLineCd"].ToString() );
					liArr[7] = new ListItem( "txtStationCd", dict["txtStationCd"].ToString() );
					e.Row.Attributes[ControlUtils.ON_DBLCLICK] = AiImageCheck.ImageInputModal.ImageCreateDispUrl( this, PageInfo.AnlGroupInput, 10, 10, liArr, FLAG_ANL_GROUP_UPDATE_DISP, "input" );

					//コピーして追加用
					btnModalCopy.Attributes[ControlUtils.ON_CLICK] = "var tmpTxt = $('#' + 'MasterBody_txtCommentCopy'); var txtLength = $(tmpTxt).val().length; var tmpTxtCp = $(tmpTxt).val();  if (txtLength > 0) {    return AiImageCheck.gridDouClickInputOpen('AnlGroupInput','./AnlGroupInput.aspx?' + tmpTxtCp + '','画像解析グループコピーして追加') } else { var tmpBtn = $('#' + 'MasterBody_btnMessage'); $(tmpBtn).trigger('click'); }";
				} 
			}
		}
		...
		private void PageIndexChanging( params object[] parameters ) {
			...
			BindGridView_WithTempField( grvDetailBodyRB, ControlUtils.GetFrozenColumns( frozenGrid, GridviewDefault, false ), ConditionInfo.ResultData );
			for ( int idx = 0; idx < grvDetailHeaderRT.Columns.Count; idx++ ) {
				string sValue = "";
				if ( false == dicDataList.TryGetValue( grvDetailHeaderRT.Columns[idx].SortExpression, out sValue ) ) {
					grvDetailHeaderRT.Columns[idx].Visible = false;
		...
		internal static void ShowGridViewHeader( GridView grv, GridViewDefine[] gridViewDefineArr, ConditionInfoSessionHandler.ST_CONDITION cond, bool isSort ) {
			...
			foreach ( GridViewDefine grvDef in gridViewDefineArr ) {
				...
				if ( dicDataList.TryGetValue( grvDef.bindField, out sValue ) ) {
		...
		internal static void BindGridView_WithTempField( GridView grv, GridViewDefine[] gridViewColumnDef, DataTable tblData ) {
			...
			foreach ( GridViewDefine grvDef in gridViewColumnDef ) {
				...
				if ( grv.Columns[colIdx] is BoundField ) {
					...
					//					Size size = TextRenderer.MeasureText( "画像解析グループコード", new Font( "Segoe UI", 12, GraphicsUnit.Pixel ), new Size(  ), TextFormatFlags.NoPadding );
					string sValue = "";
					if ( null != dicDataList ) {
						if ( true == dicDataList.TryGetValue( grvDef.bindField.ToString(), out sValue ) ) {
							int iByteNum = System.Text.Encoding.Default.GetByteCount( sValue );
		...
		internal static void MakeBoundTemplateFieldCtrl( ref TemplateField tmpField, GridViewDefine gridDef ) {
			...
			if ( null != dicDataList ) {
				if ( true == dicDataList.TryGetValue( gridDef.bindField.ToString(), out sValue ) ) {
					int iByteNum = System.Text.Encoding.Default.GetByteCount( sValue );
