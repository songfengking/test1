...
KTMonthPicker = {
    ...
    convDate: function(textval) {
        var date = new Date(textval + this.day_str);
    ...
    chkInputKey: function (ele) {
        ...
        var type = -1;                      //編集タイプ 0:年 1:月 3:年スラッシュ 5:最後尾 6:全選択
        var textval = "";                   //出力テキスト
    ...
    setFormat: function(ele, selected) {
        ...
        //日付オブジェクト作成
        var date;
        date = new Date(textval + this.day_str);
        if (isNaN(date)) {
    ...
    ,onCalendarShown:function() {
        var id = arguments.callee.caller.arguments[0]._id;
        var calCtrl = $find(id);
        calCtrl._switchMode("months", true);
        if (calCtrl._monthsBody) {
            for (var i = 0; i < calCtrl._monthsBody.rows.length; i++) {
                var row = calCtrl._monthsBody.rows[i];
                for (var j = 0; j < row.cells.length; j++) {
                    Sys.UI.DomEvent.addHandler(row.cells[j].firstChild, "click", KTMonthPicker.call);
                }
            }
        }
        return;
    ...
    ,onCalendarHidden:function() {
        ...
        if (calCtrl._monthsBody) {
            for (var i = 0; i < calCtrl._monthsBody.rows.length; i++) {
                var row = calCtrl._monthsBody.rows[i];
                for (var j = 0; j < row.cells.length; j++) {
                    Sys.UI.DomEvent.removeHandler(row.cells[j].firstChild, "click", KTMonthPicker.call);
                }
            }
        }
        return;
    ...
    ,call:function(eventElement) {
        var target = eventElement.target;
        
        switch (target.mode) {
        case "month":
            var idx = eventElement.target.id.indexOf("_month");
            var id = eventElement.target.id.substr(0,idx);

            var calCtrl = $find(id);
            calCtrl._visibleDate = target.date;
            calCtrl.set_selectedDate(target.date);
            calCtrl._switchMonth(target.date);
            calCtrl._blur.post(true);
            calCtrl.raiseDateSelectionChanged();
            break;
        }
        return;
    }
