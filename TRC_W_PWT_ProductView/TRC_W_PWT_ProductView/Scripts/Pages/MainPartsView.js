...
MainPartsView = {
    ...
    ,
    GRID_MAIN_VIEW: "divMainPartsViewScroll"
    ,
    MIN_HEIGHT: 70
    ,
    ...
    ResizeGridArea: function () {
        ...
        //グリッド用DIVサイズ変更
        var gridArea = $("#" + MainPartsView.GRID_MAIN_VIEW);
        ...
        if (0 < gridArea.length) {
            ...
            try {
                gridSize = containtsBottom - gridTop - MainPartsView.INNER_DIV_H_MARGIN;
                if (gridSize < MainPartsView.MIN_HEIGHT) {
                    //gridSize = MainPartsView.MIN_HEIGHT;
                }
    ...
    ResizeGridArea2: function () {
        ...
        if (null != containtsArea.position()) {
            containtsBottom = containtsBottomArea.position().top - containtsArea.position().top + MainPartsView.MIN_HEIGHT;
            if (null == containtsBottom) {
                containtsBottom = 0;
            }
        }
        ...
        if (0 < gridLT.length) {
            ...
            try {
                divMain_Height = containtsBottom - divMain_Top - MainPartsView.INNER_DIV_H_MARGIN;
    ...
    ,
    Initialize: function () {
        $("#" + MainPartsView.GRID_RB).scroll(function () {
            var gridRT = $("#" + MainPartsView.GRID_RT);
            var gridLB = $("#" + MainPartsView.GRID_LB);
            var gridRB = $("#" + MainPartsView.GRID_RB);
        ...
        $("#" + MainPartsView.GRID_LB).scroll(function () {
            var gridLB = $("#" + MainPartsView.GRID_LB);
            var gridRB = $("#" + MainPartsView.GRID_RB);
        ...
        $("#" + MainPartsView.GRID_LB).on(Common.GetWheelEvent(), function (e) {
            var gridLB = $("#" + MainPartsView.GRID_LB);
            gridLB.scrollTop(gridLB.scrollTop() - Common.GetWheelDelta(e));
        });
        $("#" + MainPartsView.GRID_RB).on(Common.GetWheelEvent(), function (e) {
            var gridRB = $("#" + MainPartsView.GRID_RB);
