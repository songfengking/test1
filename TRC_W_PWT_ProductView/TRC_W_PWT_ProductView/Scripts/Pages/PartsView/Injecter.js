///////////////////////////////////////////////////////////////////////////////////////////////
//詳細画面 エンジン部品:Injecter(インジェクター)用コントロール制御関数
///////////////////////////////////////////////////////////////////////////////////////////////
//メイン処理からの各画面派生呼び出し(PageLoad)
function DetailFramePageLoaded() {
    FuelInjection.Initialize();
}
//メイン処理からの各画面派生呼び出し(Resize)
function DetailFramePageResized() {
    FuelInjection.ResizeDivArea();
}

FuelInjection = {
    //定数
    DIV_BODY: "divDetailBodyScroll"
    ,   
    DIV_MAIN_AREA: "divMainListArea"
    ,
    DIV_SUB_AREA: "divSubListArea"
    ,
    PARENT_DIV_W_MARGIN: 6
    ,
    INNER_DIV_W_MARGIN: 2
    ,
    INNER_DIV_H_MARGIN: 3
    ,
    INNER_DIV_MARGIN_20: 20
    ,
    INNER_DIV_MARGIN_35: 35
    ,
    GRID_LT: "divLTScroll"
    ,
    GRID_RT: "divRTScroll"
    ,
    GRID_LB: "divLBScroll"
    ,
    GRID_RB: "divRBScroll"
    ,
    GRID_SUB_LT: "divLTSubScroll"
    ,
    GRID_SUB_RT: "divRTSubScroll"
    ,
    GRID_SUB_LB: "divLBSubScroll"
    ,
    GRID_SUB_RB: "divRBSubScroll"
    ,
    ResizeDivArea: function () {

        //詳細表示エリアDIVの位置を取得
        var containtsBottom = 0;
        var containtsWidth = 0;
        var containtsArea = $("#" + DetailFrame.DIV_VIEW_BOX);
        var containtsBottomArea = $("#" + MasterMain.CONTAINTS_AREA_BOTTOM);
        if (null != containtsArea.position()) {
            containtsBottom = containtsArea.position().top + containtsArea.height();
            if (null == containtsBottom) {
                containtsBottom = 0;
            }
            containtsWidth = containtsArea.width();
            if (null == containtsWidth) {
                containtsWidth = 0;
            }
        }

        //Div高さサイズ
        var divHSize = 0;
        //Div幅サイズ
        var divWSize = 0;
        //Div縦位置
        var divTop = 0;
        //Div横位置
        var divLeft = 0;

        //グリッド用DIVサイズ変更
        var divParentArea = $("#" + this.DIV_BODY);
//        var divMainArea = $("#" + this.DIV_MAIN_AREA);
//        var divSubArea = $("#" + FuelInjection.DIV_SUB_AREA);
                        
        //上位DIVを高さ変更
        if (0 < divParentArea.length) {
            //親DIVを幅変更する
            try {
                divWSize = containtsWidth - this.PARENT_DIV_W_MARGIN;
                divParentArea.width(divWSize);
            } catch (e) {
            }
            /*
            //内部DIVを幅変更する
            try {
                divWSize = divWSize - this.INNER_DIV_W_MARGIN;
                divMainArea.width(divWSize);
                divSubArea.width(divWSize);
            } catch (e) {
            }
            */
            //親DIVを高さ変更する
            if (null != divParentArea.position()) {
                divTop = divParentArea.position().top;
            }
            try {
                divHSize = containtsBottom - divTop - this.INNER_DIV_H_MARGIN;
                divParentArea.height(divHSize);
            } catch (e) {
            }
            /*
            //サブ側DIVを高さ変更する
            if (null != divSubArea.position()) {
                divTop = divSubArea.position().top;
            }
            try {
                divHSize = containtsBottom - divTop - this.INNER_DIV_H_MARGIN;
                divSubArea.height(divHSize);
            } catch (e) {
            }
            */
        }

        //ヘッダ固定対応

        //ウィンドウ幅
        var windowWidth = $(document).innerWidth();
        //グリッド用DIVサイズ変更
        var gridLT = $("#" + FuelInjection.GRID_LT);
        var gridRT = $("#" + FuelInjection.GRID_RT);
        var gridLB = $("#" + FuelInjection.GRID_LB);
        var gridRB = $("#" + FuelInjection.GRID_RB);

        var gridLTSub = $("#" + FuelInjection.GRID_SUB_LT);
        var gridRTSub = $("#" + FuelInjection.GRID_SUB_RT);
        var gridLBSub = $("#" + FuelInjection.GRID_SUB_LB);
        var gridRBSub = $("#" + FuelInjection.GRID_SUB_RB);

        //Table Header幅
        var tblHeaders_Width = 0;

        //Div(右)幅
        var divR_Width = 0;
        //Div(右)位置(横)
        var divR_Left = 0;

        if (0 < gridLT.length) {

            //グリッドのDIVを幅変更する
            if (null != gridLT.position()) {
                divR_Left = gridLT.position().left + gridLT.width();
            }

            try {
                tblHeaders_Width = windowWidth - this.PARENT_DIV_W_MARGIN;
                divR_Width = tblHeaders_Width - divR_Left;
                gridRT.width(divR_Width);
                gridRB.width(divR_Width);
            } catch (e) {
            }

            //mainグリッドのDIVを高さ変更する
            if (null != gridLB.position()) {
                divTop = gridLB.position().top;
            }
            try {
                divHSize = gridLTSub.position().top - divTop - this.INNER_DIV_MARGIN_35;
                gridLB.height(divHSize);
                gridRB.height(divHSize);
            } catch (e) {
            }

        }


        //Table Header幅
        tblHeaders_Width = 0;

        //Div(右)幅
        divR_Width = 0;
        //Div(右)位置(横)
        divR_Left = 0;

        if (0 < gridLTSub.length) {

            //SubグリッドのDIVを幅変更する
            if (null != gridLTSub.position()) {
                divR_Left = gridLTSub.position().left + gridLTSub.width();
            }

            try {
                tblHeaders_Width = windowWidth - this.PARENT_DIV_W_MARGIN;
                divR_Width = tblHeaders_Width - divR_Left;
                gridRTSub.width(divR_Width);
                gridRBSub.width(divR_Width);
            } catch (e) {
            }

            //SubグリッドのDIVを高さ変更する
            if (null != gridLBSub.position()) {
                divMain_Top = gridLBSub.position().top;
            }
            try {
                divMain_Height = containtsBottomArea.position().top - divMain_Top - this.INNER_DIV_MARGIN_20;
                gridLBSub.height(divMain_Height);
                gridRBSub.height(divMain_Height);
            } catch (e) {
            }
        }


    }
    ,
    //スクロール連動設定
    Initialize: function () {
        //メイン
        $("#" + FuelInjection.GRID_RB).scroll(function () {
            var gridRT = $("#" + FuelInjection.GRID_RT);
            var gridLB = $("#" + FuelInjection.GRID_LB);
            var gridRB = $("#" + FuelInjection.GRID_RB);
            if (gridRB.scrollTop() != gridLB.scrollTop()) {
                gridLB.scrollTop(gridRB.scrollTop());
            }
            if (gridRB.scrollLeft() != gridRT.scrollLeft()) {
                gridRT.scrollLeft(gridRB.scrollLeft());
            }
        });
        $("#" + FuelInjection.GRID_LB).scroll(function () {
            var gridLB = $("#" + FuelInjection.GRID_LB);
            var gridRB = $("#" + FuelInjection.GRID_RB);
            if (gridLB.scrollTop() != gridRB.scrollTop()) {
                gridRB.scrollTop(gridLB.scrollTop());
            }
        });
        $("#" + FuelInjection.GRID_LB).on(Common.GetWheelEvent(), function (e) {
            var gridLB = $("#" + FuelInjection.GRID_LB);
            gridLB.scrollTop(gridLB.scrollTop() - Common.GetWheelDelta(e));
        });
        $("#" + FuelInjection.GRID_RB).on(Common.GetWheelEvent(), function (e) {
            var gridRB = $("#" + FuelInjection.GRID_RB);
            gridRB.scrollTop(gridRB.scrollTop() - Common.GetWheelDelta(e));
        });

        //サブ
        $("#" + FuelInjection.GRID_SUB_RB).scroll(function () {
            var gridRTSub = $("#" + FuelInjection.GRID_SUB_RT);
            var gridLBSub = $("#" + FuelInjection.GRID_SUB_LB);
            var gridRBSub = $("#" + FuelInjection.GRID_SUB_RB);
            if (gridRBSub.scrollTop() != gridLBSub.scrollTop()) {
                gridLBSub.scrollTop(gridRBSub.scrollTop());
            }
            if (gridRBSub.scrollLeft() != gridRTSub.scrollLeft()) {
                gridRTSub.scrollLeft(gridRBSub.scrollLeft());
            }
        });
        $("#" + FuelInjection.GRID_LB_SUB).scroll(function () {
            var gridLBSub = $("#" + FuelInjection.GRID_SUB_LB);
            var gridRBSub = $("#" + FuelInjection.GRID_SUB_RB);
            if (gridLBSub.scrollTop() != gridRBSub.scrollTop()) {
                gridRBSub.scrollTop(gridLBSub.scrollTop());
            }
        });
        $("#" + FuelInjection.GRID_SUB_LB).on(Common.GetWheelEvent(), function (e) {
            var gridLBSub = $("#" + FuelInjection.GRID_SUB_LB);
            gridLBSub.scrollTop(gridLBSub.scrollTop() - Common.GetWheelDelta(e));
        });
        $("#" + FuelInjection.GRID_SUB_RB).on(Common.GetWheelEvent(), function (e) {
            var gridRBSub = $("#" + FuelInjection.GRID_SUB_RB);
            gridRBSub.scrollTop(gridRBSub.scrollTop() - Common.GetWheelDelta(e));
        });

    }
}