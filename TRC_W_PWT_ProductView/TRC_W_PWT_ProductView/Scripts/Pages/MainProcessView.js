...
MainProcessView = {
    ...
    ,
    GRID_MAIN_PROCESS_VIEW: "divMainProcessViewScroll"
    ,
    MIN_HEIGHT: 70
    ,
    ...
    ResizeGridArea: function () {
        ...
        //グリッド用DIVサイズ変更
        var gridArea = $("#" + MainProcessView.GRID_MAIN_PROCESS_VIEW);
        ...
        if (0 < gridArea.length) {
            ...
            try {
                gridSize = containtsBottom - gridTop - MainProcessView.INNER_DIV_H_MARGIN;
                if (gridSize < MainProcessView.MIN_HEIGHT) {
                }
    ...
    ResizeGridArea2: function () {
        ...
        if (null != containtsArea.position()) {
            containtsBottom = containtsBottomArea.position().top - containtsArea.position().top + MainProcessView.MIN_HEIGHT;
            if (null == containtsBottom) {
                containtsBottom = 0;
            }
        }
        ...
        if (0 < gridLT.length) {
            ...
            try {
                divMain_Height = containtsBottom - divMain_Top - MainProcessView.INNER_DIV_H_MARGIN;
    ...
    ,
    Initialize: function () {
        $("#" + MainProcessView.GRID_RB).scroll(function () {
            var gridRT = $("#" + MainProcessView.GRID_RT);
            var gridLB = $("#" + MainProcessView.GRID_LB);
            var gridRB = $("#" + MainProcessView.GRID_RB);
        ...
        $("#" + MainProcessView.GRID_LB).scroll(function () {
            var gridLB = $("#" + MainProcessView.GRID_LB);
            var gridRB = $("#" + MainProcessView.GRID_RB);
        ...
        $("#" + MainProcessView.GRID_LB).on(Common.GetWheelEvent(), function (e) {
            var gridLB = $("#" + MainProcessView.GRID_LB);
            gridLB.scrollTop(gridLB.scrollTop() - Common.GetWheelDelta(e));
        });
        $("#" + MainProcessView.GRID_RB).on(Common.GetWheelEvent(), function (e) {
            var gridRB = $("#" + MainProcessView.GRID_RB);
