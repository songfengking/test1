///////////////////////////////////////////////////////////////////////////////////////////////
//詳細画面 チェックシート(CheckSheet)用コントロール制御関数
///////////////////////////////////////////////////////////////////////////////////////////////
//メイン処理からの各画面派生呼び出し(PageLoad)
function DetailFramePageLoaded() {
    var viewer = new ScrollViewer("#" + CheckSheet.DIV_VIEW_BOX);
    CheckSheet.AfterImageLoad();
}
//メイン処理からの各画面派生呼び出し(Resize)
function DetailFramePageResized() {
    CheckSheet.ResizeDivArea();
}
...
CheckSheet = {
    ...
    ChangeMainAreaImage: function (ctrlId, url) {
        ...
        $("#" + ctrlId).attr("src", url);
        $("#" + CheckSheet.DIV_VIEW_BOX).scrollTop(0);
        $("#" + CheckSheet.DIV_VIEW_BOX).scrollLeft(0);

        //選択行変更
        var ele = $(window.event.srcElement);

        var listItems = $("#" + CheckSheet.DIV_LIST_AREA).find("." + CheckSheet.CSS_LIST_ITEM_DIV);
        var selectRow = ele.closest("." + CheckSheet.CSS_LIST_ITEM_DIV);
        if (0 < selectRow.length) {
            $(listItems).removeClass(CheckSheet.CSS_SELECTED_DIV);
            $(selectRow).addClass(CheckSheet.CSS_SELECTED_DIV);
        }
    }
}