///////////////////////////////////////////////////////////////////////////////////////////////
//詳細画面 画像証跡(CameraImage)用コントロール制御関数
///////////////////////////////////////////////////////////////////////////////////////////////
//メイン処理からの各画面派生呼び出し(PageLoad)
function DetailFramePageLoaded() {
    var viewer = new ScrollViewer("#" + CameraImage.DIV_VIEW_BOX);
    CameraImage.AfterImageLoad();
}
//メイン処理からの各画面派生呼び出し(Resize)
function DetailFramePageResized() {
    CameraImage.ResizeDivArea();
}
...
CameraImage = {
    ...
    ChangeMainAreaImage: function (ctrlId, url) {
        ...
        $("#" + ctrlId).attr("src", url);
        $("#" + CameraImage.DIV_VIEW_BOX).scrollTop(0);
        $("#" + CameraImage.DIV_VIEW_BOX).scrollLeft(0);

        //選択行変更
        var ele = $(window.event.srcElement);

        var listItems = $("#" + CameraImage.DIV_LIST_AREA).find("." + CameraImage.CSS_LIST_ITEM_DIV);
        var selectRow = ele.closest("." + CameraImage.CSS_LIST_ITEM_DIV);
        if (0 < selectRow.length) {
            $(listItems).removeClass(CameraImage.CSS_SELECTED_DIV);
            $(selectRow).addClass(CameraImage.CSS_SELECTED_DIV);
        }
    }
}